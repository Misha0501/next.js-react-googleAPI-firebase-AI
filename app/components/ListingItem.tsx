...
export const ListingItem = ({ listingItem, onSavedIconClick, isLoadingSavedListings = false, ownerView, onDeleteIconClick }: ListingItemProps) => {
  ...
  return (
    <div className={"rounded-lg drop-shadow-2xl bg-white h-fit"}>
      <div className="image-wrapper relative cursor-pointer h-[242px] w-full" onClick={goToListingPage}>
        {getLabel() && <div
          className="px-6 py-1 absolute top-4 left-4 bg-green-500 rounded-lg text-white">{getLabel()}</div>}
        {listingItem.ListingImage && listingItem.ListingImage[0] && <Image
        ...
        />}
        {!listingItem.ListingImage || !listingItem.ListingImage.length && <Image
          className={"h-full rounded-t-lg w-full object-cover"}
          width={384}
          height={240}
      ...
      <div className="flex justify-between p-4 gap-2 min-h-[130px]">
        <div className="flex flex-col w-full">
          <div className="mb-4 cursor-pointer" onClick={goToListingPage}>
            ...
            <span
              className={"font-bold text-gray-500"}>{listingItem.Address && listingItem.Address[0] && listingItem.Address[0].locality}</span>
          </div>
          <div className="flex gap-3 flex-wrap">
            {listingItem.areaTotal &&
              <div className="flex items-center gap-2 mb-4">
                <GridIcon></GridIcon>
                <span className={"text-gray-500"}>{listingItem.areaTotal} m²</span>
              </div>}
            {listingItem.bedrooms &&
              <div className="flex items-center gap-2 mb-4">
                <BedIcon/>
                <span className={"text-gray-500"}>{listingItem.bedrooms}</span>
              </div>}
          </div>
          {ownerView &&
