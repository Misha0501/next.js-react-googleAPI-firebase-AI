...
test.describe("Search functionality", () => {
  test("autocomplete is working and provides proper results", async ({
    page,
  }) => {
  ...
  test("should navigate to listings page once form is submitted", async ({
    page,
  }) => {
  ...
  test("should navigate to listings page once popular search is clicked", async ({
    page,
  }) => {
  ...
  test("rent/sell buttons should selected when clicked", async ({ page }) => {
    ...
    expect(await page.getAttribute(rentBtnSelector, "aria-selected")).toBe(
      "true",
    );

    // Check that the sell button is not selected
    expect(await page.getAttribute(buyBtnSelector, "aria-selected")).toBe(
      "false",
    );
  });
});
